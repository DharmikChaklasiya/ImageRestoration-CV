...
class CustomDataset(torch.utils.data.Dataset):
    def __init__(self, sorted_image_groups ):
        
        self.sorted_image_groups = sorted_image_groups
      
    def __len__(self):
        return len(self.sorted_image_groups)

    def __getitem__(self, idx):
        sorted_image_tensor_groups, image_tensor_group_map = preload_images_from_drive([self.sorted_image_groups[idx]])
        return sorted_image_tensor_groups[0].get_images()

  

class CustomDataloader(DataLoader):
    def __init__(self, sorted_image_groups ,batch_size=None, shuffle=False, seed=None):
        dataset = CustomDataset(sorted_image_groups)
        super().__init__(
            dataset,
...
class DatasetPartMetaInfo(BaseModel):
    ...
    def create_dataloaders(self, image_group_map, train_batch_size=4, val_batch_size=4,
                           eval_batch_size=1) -> Tuple[DataLoader, DataLoader, DataLoader]:

      
      
        train_img_groups = [image_group_map[i] for i in self.train_indices]
        val_img_groups = [image_group_map[i] for i in self.val_indices]
        eval_img_groups = [image_group_map[i] for i in self.eval_indices]

        # Create DataLoaders
        train_loader = CustomDataloader(train_img_groups, batch_size=train_batch_size, shuffle=True)
        val_loader = CustomDataloader(val_img_groups, batch_size=val_batch_size, shuffle=False)
        eval_loader = CustomDataloader(eval_img_groups, batch_size=eval_batch_size, shuffle=False)
...
def load_dataset_infos(all_parts,path_to_folder):
    ...
    for part in all_parts:
        ...
        if os.path.exists(file_path):
            ...
            all_image_groups, image_group_map = load_input_image_parts([part],path_to_folder)
...
def preload_images_from_drive(sorted_image_groups):
    ...
    for i, img_group in enumerate(sorted_image_groups):
