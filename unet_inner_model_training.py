...
from image_loader import load_input_image_parts, GroundTruthLabelDataset
...
def train_model_on_one_batch( batch_part: DatasetPartMetaInfo, model: nn.Module, device, super_batch_info: str, model_file_name: str ,path_to_folder:str, 
                             train_batch_size : int= 4, val_batch_size:int = 4,eval_batch_size:int =1,epochs:int = 20):
    
    sorted_image_groups, image_group_map = load_input_image_parts([batch_part.part_name],path_to_folder)
   
    loss_history: LossHistory = batch_part.get_loss_history(model_file_name)
    train_loader, val_loader, eval_dataloader = batch_part.create_dataloaders(image_group_map,train_batch_size, val_batch_size,
                           eval_batch_size)
